...
def clockwise_scanned(x, y, w, h, asteroids):
    ...
    lines = chain(scans, straight_lines)
    angle_sorted = sorted(lines, key=lambda p: pseudoangle((-p[0], -p[1])), reverse=True)
    angle_sorted = chain([(0, 1)], angle_sorted)

    result = []
    for w_s, h_s in angle_sorted:
        # ugly hack to correct the discrepancy between coordinates and the "up" direction
        h_s = -h_s
...
x_max, y_max = max(asteroids, key=lambda p: count_visible_from(p[0], p[1], width, height, asteroids))
...
while True:
    ...
    left_to_shoot = [(n - n_shot, islice(it, n_shot, None)) for n, it in left_to_shoot if n > n_shot]
    n_left -= n_together
